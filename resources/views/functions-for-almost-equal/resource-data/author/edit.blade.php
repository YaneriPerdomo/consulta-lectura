...
<body class="h-100 d-flex flex-column">
    ...
    <main class="flex__grow-2 flex-full__aligh-start">
        <form action="{{ route('employee.author.update', $data_author->slug) }}" method="post" class="form form--employee">
            @csrf
                    ...
                    <span class="form__icon input-group-text p-0 w-100" id="basic-addon1">
                        <div class="input-group">
                            <select id="gender_id" name="gender_id" class="form-control form__select " required>
                                ...
                                @if ($data_author)
                                        <option value="1" @if($data_author->gender_id == "1") selected @endif> F </option>
                                        <option value="2" @if($data_author->gender_id == "2") selected @endif> M</option>
                                    @else
