        ...
        <article class="form  w-adjustable align-self-start">
            <div class="flex-full__justify-content-between p-0">
                ...
                @if (Auth::check() && Auth::user()->rol_id == 3)
                    <div>
                        <a href="{{ route('employee.tag.create') }}">
            ...
            <div class="">
                            ...
                            @else
                                @foreach ($tags->items() as $value)
                                    <tr class='show'>
                                        ...
                                        <td class='operations'>
                                            <a href="{{ route('employee.tag.delete', $value->slug) }}">
                                                <button type="button" class="button button--color-red js-detele-account">
                                                    <i class='bi bi-trash''></i>

                                                                    </button></a>
                                                                    <a href=' {{ route('employee.tag.edit', $value->slug ?? null) }}'>
                ...
                <div class="flex-full__justify-content-between">
                    <div>
                        <p>
                            Mostrando {{ $tags->count() > 0 ? 'registros' : 'registro' }}
                            {{ $tags->count() }} - {{ $tags->count() }}
                            de un total de {{ $tags->total() }}
                        </p>
                    </div>
                    <div>
                        {{ $tags->links() }}
